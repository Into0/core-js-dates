...
function getNextFriday(date) {
  const nextFriday = (5 - date.getDay() + 7) % 7 || 7;
  return new Date(date.setDate(date.getDate() + nextFriday));
}
...
function getWeekNumberByDate(date) {
  ...
  const daysPast = Math.floor((date - yearStart) / (1000 * 3600 * 24));
  return Math.floor((daysPast + 7) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  for (; month < 12; month += 1) {
    if (new Date(year, month, 13).getDay() === 5) break;
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (startDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      ...
      result.push(startDate.toLocaleDateString('en-gb').replace(/(\/)/g, '-'));
